...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    for (int i = 0; i < duck.nfaces(); i++) {
      ...
      if (duck.ray_triangle_intersect(i, orig, dir, dist_i) && dist_i < duck_dist) {
        duck_dist = dist_i;
        hit = orig + dir * dist_i;
        Vec3f t0 = duck.point(duck.vert(i,0));
        Vec3f t1 = duck.point(duck.vert(i,1));
        Vec3f t2 = duck.point(duck.vert(i,2));

        N = cross(t1-t0, t2-t0).normalize();
        material =  Material(1.5, Vec4f(0.0,  0.5, 0.1, 0.8), Vec3f(0.8, 0.8, 0),  125.);
      }
    }

    return std::min(spheres_dist, checkerboard_dist)<1000 || duck_dist < 1000;
}
